...
**Bug de spacing doble corregido (14 ago 2026, mismo hallazgo que Non Template):** `SKILLS` y `CERTIFICATIONS & TRAININGS` son títulos con `before=240/after=240` cada uno — si `skills_items` viene vacío, el gap entre `PROFESSIONAL EXPERIENCE` y `CERTIFICATIONS & TRAININGS` se duplica a 480 twips (Word suma `after`+`before` entre párrafos adyacentes, no los colapsa). Corregido con el mismo mecanismo ya usado en Non Template: título + loop de `skills_items` y de `certifications_items` envueltos cada uno en su propio `{% if %}`/`{% endif %}` como párrafos `CV Control Tag` — la sección entera desaparece cuando la lista está vacía. Verificado renderizando el fixture de Shirley Mercado con `certifications: []`: la sección desaparece, cero párrafos vacíos. 75 tests + smoke 6/6 en verde.
...
**Bug real reportado por FITS y corregido (27 ago 2026, feedback sobre candidato real Bryan Pagán Díaz):** el footer de página no mostraba el nombre del candidato — salía el texto literal `"Name"`. Causa: al anotar la plantilla con los tags Jinja, el footer nunca se tocó — quedó con el placeholder original de Word, nunca reemplazado por `{{ full_name }}` (New Format sí lo tenía correcto desde el inicio). Corregido reemplazando el texto literal por `{{ full_name }}` en `word/footer1.xml` de ambas copias del `.docx` (`microservicio/templates/anotados/`, la que usa producción, y la copia de referencia fuera de `microservicio/`). **Se confirmó que BD Format tiene el footer completamente vacío (sin nombre ni paginación) — decisión explícita de Santiago (27 ago 2026): así está bien, no requiere ese footer, no se toca.** De paso se encontró que `smoke_test.py` no podía haber detectado este bug nunca: usaba `d.paragraphs` de `python-docx`, que no incluye headers/footers de sección (mismo punto ciego que tuvo `extract.py`, corregido ahí el 25 ago pero del lado de la extracción, no del render) — corregido agregando `section.header.paragraphs`/`section.footer.paragraphs` de cada sección al texto verificado. Confirmado empíricamente que el smoke test nuevo detecta el bug: reintroducido el placeholder `"Name"` en una copia temporal del template, el chequeo de `full_name` en el footer falla como se espera. 86 tests + smoke 6/6 en verde tras el fix.
...
**Calibración fina (14 ago 2026), mismo método que BD Format (medición glyph-box con `pymupdf` sobre los 2 mismos canon, Andrea I. Arzola Torres y Aneira Palmer Berrios) — mismas secciones/orden de la sección anterior, sin cambios de contenido, solo layout:**

- **Spacing roto (bug encontrado, el más importante de esta ronda):** a diferencia de New Format y BD, este template **no tenía ningún `w:before` explícito en ningún párrafo** — solo `w:after="0"` suelto en 7 párrafos, y `pPrDefault`/estilo `Normal` sin spacing propio. Efecto real: título de sección pegado al bloque anterior, contenido pegado al título, y una empresa pegada a la siguiente en `PROFESSIONAL EXPERIENCE`, sin ningún respiro — contradice ambos canon, que muestran gaps de bloque consistentes (~240 twips, medidos glyph-a-glyph, idéntico en magnitud al ya calibrado en New Format/BD). Corregido con el mismo modelo de 2 niveles: `before=240/after=0` en cada título de sección, primer item de educación y `company.header`; `before=240/after=240` en los títulos que preceden contenido de párrafo libre (`SUMMARY OF QUALIFICATIONS`, `SKILLS`, `LICENSES, TRAININGS & CERTIFICATIONS`); todo lo demás (bullets, institución bajo grado, `role.header`) queda en `before=0/after=0`, apretado dentro del bloque.
- **Bullets sin sangría colgante real (bug encontrado):** igual que el bug ya corregido en BD Format — el guion arrancaba en x=90pt con un espacio literal antes del texto (`"- {{ b }}"` en un solo run), así que la línea envuelta de un bullet de 2+ líneas volvía a x=90pt en vez de alinearse bajo el texto. Corregido con el mismo patrón exacto de BD: `ind left=720 hanging=360`, tab stop real a 720 (`w:tabs`), y el guion separado del campo en 2 runs con un `<w:tab/>` real entremedio — aplicado a los 3 loops con bullet literal (`role.bullets`, `certifications_items`, `skills_items`).
- **Bullets de experiencia no justificados (bug encontrado):** confirmado por los 2 canon — líneas envueltas de bullets multi-línea llegan al mismo margen derecho (x≈542pt en ambos PDF, decenas de líneas medidas), es decir texto justificado (`jc="both"`), que la plantilla no tenía. Corregido solo en `role.bullets` (los bullets de `certifications_items`/`skills_items` son entradas cortas de una sola línea en los 2 canon, sin evidencia de que necesiten justificado — se dejaron sin `jc` para no introducir un cambio sin respaldo).
- **`role.header` sin tab stop (bug encontrado, sin impacto visible en los 2 canon):** ninguno de los 2 ejemplos tiene 2+ roles agrupados bajo la misma empresa, así que `role.header` nunca lleva período propio en la práctica — pero la plantilla no tenía definido el tab stop derecho (`w:tabs pos=9360`) que sí usa `company.header` para ese caso. Agregado por paridad con New Format (mismo `role_period_style="tab"`), para que el día que aparezca un candidato con 2+ roles en la misma empresa el período se alinee al margen derecho en vez de caer en el tab stop por defecto de Word.
- **Header con años de experiencia (decisión de contenido, no bug de layout):** ninguno de los 2 canon de Non Template muestra la línea de años de experiencia en el header (a diferencia de New Format/Yanina, que sí la tiene) — confirmado revisando el header completo de ambos PDF, sin ningún span adicional junto al nombre. Decisión explícita de Santiago (14 ago 2026): quitarla de Non Template. El header queda `{{ full_name }}` solo, sin tab ni condicional de `years_experience` (el campo se sigue calculando y extrayendo igual, solo no se renderiza en este template).
- **Verificado con candidata real (14 ago 2026):** Yajaira Ortiz Ruiz (`candidateId 422900145`, job "Process Development Scientist -04" `jobId 10957423`, etapa real "8. CONVERT RESUME-NON TEMPLATE") — encontrada vía `api.jazz.co` con la cookie de sesión de Santiago (nunca guardada en archivo), pipeline completo corrido en local (`extract.py` → `agent.py`, llamada real a Claude, → `blocks.py` → render), sin N8N. `state=review`, sin errores de grounding. Candidato con 20 años de experiencia en 5 empresas y **`certifications: []`** (sin certificaciones en el CV original) — este caso encontró el siguiente bug real.
- **Espaciado doble cuando una sección con loop queda vacía (bug encontrado con este candidato):** `LICENSES, TRAININGS & CERTIFICATIONS` (antes de `certifications: []`) y `SKILLS` son títulos consecutivos, cada uno con `before=240/after=240`. Word **no colapsa** `spacing before`/`after` entre párrafos adyacentes (a diferencia de CSS margin-collapse) — los suma. Con el loop de certificaciones vacío, no queda ningún párrafo de contenido entre los 2 títulos para "absorber" el `after=240` del primero, así que el gap real es `240 (after de LICENSES) + 240 (before de SKILLS) = 480` twips, el doble del gap normal — visible como un hueco anormalmente grande en el render de Yajaira. `EDUCATION`/`PROFESSIONAL EXPERIENCE` no tienen este riesgo porque usan el patrón contrario (`after=0` en el título, `before=240` en el primer item de contenido) — inmune por diseño, ya que un loop vacío ahí simplemente no aporta gap extra. **Mismo bug latente en New Format** (mismo patrón `before=240/after=240` en sus títulos `SKILLS`/`CERTIFICATIONS`), no disparado todavía porque ningún candidato de prueba tuvo esa lista vacía — fuera de alcance arreglarlo ahí en esta ronda.
  - **Corregido en Non Template:** título + loop de `certifications_items` y de `skills_items` envueltos cada uno en su propio `{% if %}`/`{% endif %}` como párrafos `CV Control Tag` separados (mismo mecanismo que el fix de `town` en BD Format) — cuando la lista está vacía, Jinja elimina la sección completa (título incluido), y el gap entre las 2 secciones vecinas queda en un solo `240` (el `before` de la siguiente sección que sí tiene contenido), nunca duplicado. No se tocaron los valores antes/después de las secciones (siguen antes/después = 240/240 cuando hay contenido).
- **Verificado:** 74 tests no-`llm` + smoke test 6/6 en verde tras ambos fixes; render real de punta a punta con Shirley Mercado y con Yajaira Ortiz Ruiz confirma header sin años, sin `None`, sin tags sueltos, y el gap `LICENSES → SKILLS` correcto (sencillo, no doble) con `certifications: []`.
- **Pendiente antes de dar esto por cerrado:** confirmación visual de Santiago sobre los `.docx` regenerados, y verificación vía Microsoft Graph (el método que sí se hizo para New Format y BD Format) — no se hizo en esta ronda.
