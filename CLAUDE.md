...
**Fase 6 completa a nivel de construcción (4 ago 2026), pendiente de activación:** poller (`JazzHR - Convert Resume Poller`) y worker (`JazzHR - Convert Resume Processor`) construidos y probados en `fits.app.n8n.cloud` vía API de n8n, replicando el patrón ya probado del poller/worker de AI Screening (Fase 1). Cubren solo Módulo 2 (detección de candidato en etapa Convert Resume → descarga de CV → `/transform` → `/render`) — Módulo 3 (SharePoint, email, log de Sheets) queda para otra sesión. Mapeo completo de los 3 espacios de IDs de JazzHR (workflowId interno / workflow_id público / step_id) documentado en `conocimiento/jazzhr-stepids-convert-resume.md`. Verificado con la API real de JazzHR: **ningún candidato está todavía en ninguna de las 13 etapas Convert Resume** — ambos workflows quedan **desactivados** a pedido de Santiago hasta que FITS empiece a moverlos ahí. **Hardening adicional (6 ago 2026):** choque de scheduling entre pollers corregido (Interval → CronExpression, ventana de 15 min), riesgo de OOM cerrado con `MAX_PER_CYCLE` (mismo patrón que AI Screening Poller), y ambos workflows vinculados a `Error Alert Global`. Sigue sin haber candidatos en las etapas Convert Resume. Detalle completo de bugs encontrados/corregidos en `seguimiento/bitacora.md` (4 y 6 ago).

**Falta:** activar el poller/worker de Fase 6 cuando FITS confirme uso de las etapas Convert Resume; construir Módulo 3 (SharePoint + email + Sheets, incluyendo crear el Sheet de log que todavía no existe — ver `03-modulo3-entrega-equipo/log-sheets-diseno.md`); set de 15–20 CVs reales de FITS para medir falsos positivos del grounding y ajuste fino (Fase 7); confirmación visual de FITS/Paola sobre los 3 formatos (paso de negocio, no bloquea el uso del microservicio). **Worksense Format descartado por FITS** (Paola, 21 jul 2026) y su archivo eliminado del repo. Detalle completo en `Decisiones.md`, `agente/CONTRATO-AGENTE.md`, `conocimiento/` y `seguimiento/bitacora.md`.
...
2. Agente de Transformación AI (Anthropic/Claude + microservicio de render — 4 reglas de contenido + generación de .docx con branding, 3 formatos activos) — **desplegado en Railway, wiring en N8N (poller+worker) construido y probado, desactivado hasta que FITS empiece a usar las etapas**
