...
**Fase 6 completa a nivel de construcción (4 ago 2026), DESACTIVADA — bloqueada por falta de Módulo 3:** poller (`JazzHR - Convert Resume Poller`) y worker (`JazzHR - Convert Resume Processor`) construidos y probados en `fits.app.n8n.cloud` vía API de n8n, replicando el patrón ya probado del poller/worker de AI Screening (Fase 1). Cubren solo Módulo 2 (detección de candidato en etapa Convert Resume → descarga de CV → `/transform` → `/render`) — el Processor termina en `POST /render` y devuelve el `.docx` por el webhook sin persistirlo en ningún lado, porque Módulo 3 (SharePoint, email, log de Sheets) no está construido. Mapeo completo de los 3 espacios de IDs de JazzHR (workflowId interno / workflow_id público / step_id) documentado en `conocimiento/jazzhr-stepids-convert-resume.md`. **Hardening (6 ago 2026):** choque de scheduling entre pollers corregido (Interval → CronExpression, ventana de 15 min), riesgo de OOM cerrado con `MAX_PER_CYCLE` (mismo patrón que AI Screening Poller), y ambos workflows vinculados a `Error Alert Global`. **Intento de activación (11 ago 2026):** FITS confirmó a Santiago que ya movieron candidatos a las etapas Convert Resume; se activaron ambos workflows vía API de n8n pero se revirtió en la misma sesión al confirmar que no había destino para el `.docx` generado — sin evidencia de que corriera ningún ciclo real en la ventana breve en que estuvieron activos. **No reactivar hasta que Módulo 3 esté construido y probado.** Detalle completo en `seguimiento/bitacora.md` (4, 6 y 11 ago).

**Falta:** construir Módulo 3 (SharePoint + email + Sheets, incluyendo crear el Sheet de log que todavía no existe — ver `03-modulo3-entrega-equipo/log-sheets-diseno.md`) — **bloqueante para reactivar Fase 6**; set de 15–20 CVs reales de FITS para medir falsos positivos del grounding y ajuste fino (Fase 7); confirmación visual de FITS/Paola sobre los 3 formatos (paso de negocio, no bloquea el uso del microservicio). **Worksense Format descartado por FITS** (Paola, 21 jul 2026) y su archivo eliminado del repo. Detalle completo en `Decisiones.md`, `agente/CONTRATO-AGENTE.md`, `conocimiento/` y `seguimiento/bitacora.md`.
...
2. Agente de Transformación AI (Anthropic/Claude + microservicio de render — 4 reglas de contenido + generación de .docx con branding, 3 formatos activos) — **desplegado en Railway, wiring en N8N (poller+worker) construido y probado, desactivado — bloqueado hasta que exista Módulo 3 (destino del .docx)**
3. Entrega al Equipo (.docx + email grupal, almacenamiento en SharePoint) — **no iniciado**
