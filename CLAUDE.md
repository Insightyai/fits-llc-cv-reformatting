...
**Falta:** deploy a Railway, integración en N8N (Fase 6 — trigger, descarga del CV, wiring `/transform`→`/render`), set de 15–20 CVs reales de FITS para medir falsos positivos del grounding y ajuste fino (Fase 7), y la confirmación visual de FITS/Paola sobre los 3 formatos (paso de negocio, no bloquea el uso del microservicio). **Worksense Format descartado por FITS** (Paola, 21 jul 2026) y su archivo eliminado del repo. Detalle completo en `Decisiones.md`, `agente/CONTRATO-AGENTE.md`, `conocimiento/` y `seguimiento/bitacora.md`.
...
2. Agente de Transformación AI (Anthropic/Claude + microservicio de render — 4 reglas de contenido + generación de .docx con branding, 3 formatos activos) — **en progreso, Fases 0–5 de 7 completas**
